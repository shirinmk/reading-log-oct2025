...
const AdminLibraryDashboard = forwardRef((props, ref) => {
  const [books, setBooks] = useState([]);
  const [borrowed, setBorrowed] = useState([]);
  const [readers, setReaders] = useState([]);
  const [message, setMessage] = useState("");
  ...
  useImperativeHandle(ref, () => ({
    exportData() {
      if (activeTab === "books") {
        ...
        exportToExcel(data, "Library_Books");
      } else if (activeTab === "borrowed") {
        if (!borrowed.length) return alert("No borrowed books to export.");
        const data = borrowed.map((r) => ({
          Book: r.book?.title || "—",
          Reader:
            r.reader?.firstName && r.reader?.lastName
              ? `${r.reader.firstName} ${r.reader.lastName}`
              : "—",
          BorrowedAt: new Date(r.borrowedAt).toLocaleDateString(),
          DueDate: r.dueDate ? new Date(r.dueDate).toLocaleDateString() : "—",
          Status: r.status,
        }));
        exportToExcel(data, "Borrowed_Books");
      }
    },
  }));
