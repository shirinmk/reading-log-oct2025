...
const AdminReports = forwardRef((props, ref) => {
  const navigate = useNavigate();
  const user = JSON.parse(localStorage.getItem("user"));
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      api.get("/admin/reports/readers"),
      api.get("/admin/reports/by-teacher"),
      api.get("/admin/reports/by-grade"),
    ])
      .then(([a, b, c]) => {
        if (!mounted) return;
        setStudents(a.data || []);
      ...
      .catch((e) => console.error("Admin reports load error:", e))
      .finally(() => mounted && setLoading(false));
    return () => (mounted = false);
  }, []);

  // ✅ Excel export logic for each tab
  useImperativeHandle(ref, () => ({
    exportData() {
      if (tab === "students") {
        if (!filteredStudents.length) return alert("No student data to export.");
        const data = filteredStudents.map((r, i) => ({
          "#": i + 1,
          Name: `${r.firstName} ${r.lastName}`,
          Teacher: r.teacher || "",
          Grade: r.grade || "",
          "Pages Read": r.pagesRead ?? 0,
        }));
        exportToExcel(data, "Students_Report");
      } else if (tab === "teachers") {
        if (!byTeacher.length) return alert("No teacher data to export.");
        const data = byTeacher.map((r, i) => ({
          "#": i + 1,
          Teacher: r.teacher || "",
          Students: r.students,
          "Total Pages": r.totalPages,
          "Avg Pages": r.avgPages,
          "Top Reader":
            r.topReader &&
            `${r.topReader.firstName} ${r.topReader.lastName} (${r.topReader.pagesRead ?? 0})`,
        }));
        exportToExcel(data, "Teacher_Report");
      } else if (tab === "grades") {
        if (!byGrade.length) return alert("No grade data to export.");
        const data = byGrade.map((r, i) => ({
          "#": i + 1,
          Grade: r.grade || "",
          Students: r.students,
          "Total Pages": r.totalPages,
        ...
        exportToExcel(data, "Grade_Report");
      }
    },
  }));
                  ...
                  {loading ? (
                    ...
                    byTeacher.map((row, i) => (
                      <tr key={`${row.teacher}-${i}`}>
                        ...
                        <td>
                          {row.topReader
                            ? `${row.topReader.firstName} ${row.topReader.lastName} (${row.topReader.pagesRead ?? 0})`
                            : "—"}
                        </td>
                      </tr>
                    ))
